...
class zzng extends AbstractMap {
    ...
    private final int zzk(Comparable comparable) {
        ...
        int i2 = 0;
        if (i >= 0) {
            int iCompareTo = comparable.compareTo(((zzna) this.zzb.get(i)).zza());
            if (iCompareTo > 0) {
                return -(size + 1);
            }
            if (iCompareTo == 0) {
        ...
        while (i2 <= i) {
            int i3 = (i2 + i) / 2;
            int iCompareTo2 = comparable.compareTo(((zzna) this.zzb.get(i3)).zza());
            if (iCompareTo2 < 0) {
                i = i3 - 1;
            } else {
                if (iCompareTo2 <= 0) {
    ...
    public final int hashCode() {
        ...
        int iHashCode = 0;
        for (int i = 0; i < iZzb; i++) {
            iHashCode += ((zzna) this.zzb.get(i)).hashCode();
        }
