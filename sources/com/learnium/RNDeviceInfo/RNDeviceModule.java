...
public class RNDeviceModule extends ReactContextBaseJavaModule {
    ...
    public Map<String, Object> getConstants() {
        ...
        map.put(Constants.DEVICE_ID_TAG, Build.BOARD);
    ...
    public boolean isEmulatorSync() {
        return Build.FINGERPRINT.startsWith("generic") || Build.FINGERPRINT.startsWith("unknown") || Build.MODEL.contains("google_sdk") || Build.MODEL.toLowerCase(Locale.ROOT).contains("droid4x") || Build.MODEL.contains("Emulator") || Build.MODEL.contains("Android SDK built for x86") || Build.MANUFACTURER.contains("Genymotion") || Build.HARDWARE.contains("goldfish") || Build.HARDWARE.contains("ranchu") || Build.HARDWARE.contains("vbox86") || Build.PRODUCT.contains("sdk") || Build.PRODUCT.contains("google_sdk") || Build.PRODUCT.contains("sdk_google") || Build.PRODUCT.contains("sdk_x86") || Build.PRODUCT.contains("vbox86p") || Build.PRODUCT.contains(com.google.firebase.firestore.BuildConfig.TARGET_BACKEND) || Build.PRODUCT.contains(Device.JsonKeys.SIMULATOR) || Build.BOARD.toLowerCase(Locale.ROOT).contains("nox") || Build.BOOTLOADER.toLowerCase(Locale.ROOT).contains("nox") || Build.HARDWARE.toLowerCase(Locale.ROOT).contains("nox") || Build.PRODUCT.toLowerCase(Locale.ROOT).contains("nox") || Build.SERIAL.toLowerCase(Locale.ROOT).contains("nox") || (Build.BRAND.startsWith("generic") && Build.DEVICE.startsWith("generic"));
    }
    ...
    public boolean isCameraPresentSync() {
        try {
            return ((CameraManager) getReactApplicationContext().getSystemService(PermissionsTracker.CAMERA)).getCameraIdList().length > 0;
        } catch (Exception unused) {
            return false;
        }
    }
    ...
    public boolean isBatteryChargingSync() {
        ...
        return intentRegisterReceiver != null && intentRegisterReceiver.getIntExtra("status", -1) == 2;
    }
    ...
    public WritableMap getPowerStateFromIntent(Intent intent) {
        ...
        String str = intExtra3 == 0 ? "unplugged" : intExtra4 == 2 ? Device.JsonKeys.CHARGING : intExtra4 == 5 ? IdentityInteractor.FULL_SDK_VERSION_SUFFIX : "unknown";
