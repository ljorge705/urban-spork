...
public final class ObservableFromStream<T> extends Observable<T> {
    ...
    static final class StreamDisposable<T> implements QueueDisposable<T> {
        ...
        public void run() {
            ...
            while (!this.disposed) {
                try {
                    cctxrwizduxmjefyvyrx cctxrwizduxmjefyvyrxVar = (Object) Objects.requireNonNull(it.next(), "The Stream's Iterator.next returned a null value");
                    if (!this.disposed) {
                        observer.onNext(cctxrwizduxmjefyvyrxVar);
                        if (!this.disposed) {
                            try {
                                if (!it.hasNext()) {
                                    observer.onComplete();
