...
public enum MoonPhase {
    ...
    public Moment before(Moment moment) {
        ...
        if (i >= estimatedLunations) {
            while (momentAtLunation.plus(29L, (long) TimeUnit.DAYS).isBefore((UniversalTime) moment)) {
                i++;
                Moment momentAtLunation2 = atLunation(i);
                if (!momentAtLunation2.isBefore((UniversalTime) moment)) {
                    break;
    ...
    public Moment atLunation(int i) {
        ...
        return (Moment) JulianDay.ofEphemerisTime(dSin).toMoment().with((ChronoElement<ChronoElement<TimeUnit>>) Moment.PRECISION, (ChronoElement<TimeUnit>) TimeUnit.SECONDS);
    }
    ...
    private double periodic24(double d, double d2, double d3, double d4) {
        ...
        for (int i = 23; i >= 0; i--) {
            ...
            dSin += d5 * sin((iArr2[i] * d2) + (iArr3[i] * d3) + (iArr4[i] * d4));
        }
        return dSin;
    }
