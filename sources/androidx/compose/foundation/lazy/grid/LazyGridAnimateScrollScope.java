...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.isVertical$foundation_release()) {
            iM4508getXimpl = IntOffset.m4509getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM4508getXimpl = IntOffset.m4508getXimpl(lazyGridItemInfo2.getOffset());
        }
        return Integer.valueOf(iM4508getXimpl);
    }
    ...
    private final int calculateLineAverageMainAxisSize(final List<? extends LazyGridItemInfo> visibleItems, final boolean isVertical) {
        ...
        Function1<Integer, Integer> function1 = new Function1<Integer, Integer>() { // from class: androidx.compose.foundation.lazy.grid.LazyGridAnimateScrollScope$calculateLineAverageMainAxisSize$lineOf$1
            ...
            public final Integer invoke(int i) {
                return Integer.valueOf(isVertical ? visibleItems.get(i).getRow() : visibleItems.get(i).getColumn());
            }
        };
        int i = 0;
        int i2 = 0;
        ...
        while (i < visibleItems.size()) {
            ...
            if (iIntValue == -1) {
                ...
                int iMax = 0;
                while (i < visibleItems.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (isVertical) {
                        iM4550getWidthimpl = IntSize.m4549getHeightimpl(visibleItems.get(i).getSize());
                    } else {
                        iM4550getWidthimpl = IntSize.m4550getWidthimpl(visibleItems.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM4550getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
