...
public final class LazyGridSpanLayoutProvider {
    ...
    private final List<GridItemSpan> getDefaultSpans(int currentSlotsPerLine) {
        ...
        for (int i = 0; i < currentSlotsPerLine; i++) {
            arrayList.add(GridItemSpan.m816boximpl(LazyGridSpanKt.GridItemSpan(1)));
        }
    ...
    */
    ...
    public final int m865getLineIndexOfItem_Ze7BM(final int itemIndex) {
        int i = 0;
        if (getTotalSize() <= 0) {
            return LineIndex.m874constructorimpl(0);
        }
        if (itemIndex >= getTotalSize()) {
            throw new IllegalArgumentException("Failed requirement.".toString());
        }
        if (!this.itemProvider.getHasCustomSpans()) {
            return LineIndex.m874constructorimpl(itemIndex / this.slotsPerLine);
        }
        ...
        return LineIndex.m874constructorimpl(bucketSize);
    }
    ...
    public final int spanOf(int itemIndex, int maxSpan) {
        ...
        return RangesKt.coerceIn(GridItemSpan.m820getCurrentLineSpanimpl(lazyGridItemProvider.mo850getSpan_orMbw(lazyGridItemSpanScopeImpl, itemIndex)), 1, this.slotsPerLine);
    }
