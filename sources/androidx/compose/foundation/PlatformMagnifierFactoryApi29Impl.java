...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        long jMo578toSizeXkaWNTQ = density.mo578toSizeXkaWNTQ(style.getSize());
        float fMo577toPx0680j_4 = density.mo577toPx0680j_4(style.getCornerRadius());
        float fMo577toPx0680j_42 = density.mo577toPx0680j_4(style.getElevation());
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo578toSizeXkaWNTQ != Size.INSTANCE.m1716getUnspecifiedNHjbRc()) {
            builder.setSize(MathKt.roundToInt(Size.m1708getWidthimpl(jMo578toSizeXkaWNTQ)), MathKt.roundToInt(Size.m1705getHeightimpl(jMo578toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo577toPx0680j_4)) {
            builder.setCornerRadius(fMo577toPx0680j_4);
        }
        if (!Float.isNaN(fMo577toPx0680j_42)) {
            builder.setElevation(fMo577toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo492updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m1658isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m1639getXimpl(sourceCenter), Offset.m1640getYimpl(sourceCenter), Offset.m1639getXimpl(magnifierCenter), Offset.m1640getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m1639getXimpl(sourceCenter), Offset.m1640getYimpl(sourceCenter));
            }
        }
    }
}
