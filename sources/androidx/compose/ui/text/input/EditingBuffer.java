...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m3903getMinimpl(j);
        this.selectionEnd = TextRange.m3902getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM3903getMinimpl = TextRange.m3903getMinimpl(j);
        int iM3902getMaximpl = TextRange.m3902getMaximpl(j);
        if (iM3903getMinimpl < 0 || iM3903getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM3903getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM3902getMaximpl < 0 || iM3902getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM3902getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM3903getMinimpl > iM3902getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM3903getMinimpl + " > " + iM3902getMaximpl);
        }
    }
    ...
    public final TextRange m4035getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m3893boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m4036getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM4037updateRangeAfterDeletepWDy79M = EditingBufferKt.m4037updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m3903getMinimpl(jM4037updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m3902getMaximpl(jM4037updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM4037updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4037updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m3899getCollapsedimpl(jM4037updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m3903getMinimpl(jM4037updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m3902getMaximpl(jM4037updateRangeAfterDeletepWDy79M2);
            }
        }
    }
