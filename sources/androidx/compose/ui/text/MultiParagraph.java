...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m4348getMinWidthimpl(j) != 0 || Constraints.m4347getMinHeightimpl(j) != 0) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m4341getHasBoundedHeightimpl(j)) {
                iM4345getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m4345getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM4345getMaxHeightimpl = Constraints.m4345getMaxHeightimpl(j);
            }
            Paragraph paragraphM3810Paragraph_EkL_Y = ParagraphKt.m3810Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, iM4346getMaxWidthimpl, 0, iM4345getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + paragraphM3810Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + paragraphM3810Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(paragraphM3810Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (paragraphM3810Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m4346getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m3796paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m1913getUnspecified0d7_KjU();
        }
        multiParagraph.m3801paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }

    /* renamed from: paint-RPmYEkk, reason: not valid java name */
    public final void m3801paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo3773paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m3800paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo3772paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m3802painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m4119drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int iFindParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); iFindParagraphByIndex < size; iFindParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                Path.m2139addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final int m3798getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m1640getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m1640getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m1640getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo3770getOffsetForPositionk4lQ0M(paragraphInfo.m3807toLocalMKHz9U(position)));
    }
    ...
    public final long m3799getWordBoundaryjx7JFs(int offset) {
        int iFindParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            iFindParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(iFindParagraphByIndex);
        return paragraphInfo.m3806toGlobalGEjPoXI(paragraphInfo.getParagraph().mo3771getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
