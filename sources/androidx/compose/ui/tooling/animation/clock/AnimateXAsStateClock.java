...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    public AnimateXAsStateClock(AnimateXAsStateComposeAnimation<T, V> animation) {
        ...
        this.state = new TargetState<>(getAnimation().m4316getAnimationObject().getValue(), getAnimation().m4316getAnimationObject().getValue());
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m4316getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m4316getAnimationObject().getVelocity());
    }
}
