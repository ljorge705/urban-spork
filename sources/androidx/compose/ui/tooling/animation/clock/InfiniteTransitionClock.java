...
public final class InfiniteTransitionClock implements ComposeAnimationClock<InfiniteTransitionComposeAnimation, TargetState<Object>> {
    ...
    public List<ComposeAnimatedProperty> getAnimatedProperties() {
        List<InfiniteTransition.TransitionAnimationState<?, ?>> animations = getAnimation().m4319getAnimationObject().getAnimations();
    ...
    public long getMaxDurationPerIteration() {
        Long l;
        Iterator<T> it = getAnimation().m4319getAnimationObject().getAnimations().iterator();
        if (it.hasNext()) {
    ...
    public List<TransitionInfo> getTransitions(long stepMillis) {
        ...
        Iterator<T> it = animations.iterator();
        while (it.hasNext()) {
            arrayList.add(UtilsKt.createTransitionInfo((InfiniteTransition.TransitionAnimationState) it.next(), stepMillis, getMaxDuration()));
        }
        ArrayList arrayList2 = new ArrayList();
        for (Object obj : arrayList) {
    ...
    private final <T, V extends AnimationVector> long getIterationDuration(InfiniteTransition.TransitionAnimationState<T, V> transitionAnimationState) {
        ...
        return UtilsKt.millisToNanos(vectorizedDurationBasedAnimationSpecVectorize.getDelayMillis() + (vectorizedDurationBasedAnimationSpecVectorize.getDurationMillis() * i));
    }
}
