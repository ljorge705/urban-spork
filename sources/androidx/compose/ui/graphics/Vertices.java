...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m1931toArgb8_81llA(colors.get(i).m1887unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM1640getYimpl = Offset.m1639getXimpl(packedValue);
            } else {
                fM1640getYimpl = Offset.m1640getYimpl(packedValue);
            }
            fArr[i] = fM1640getYimpl;
        }
        return fArr;
    }
}
