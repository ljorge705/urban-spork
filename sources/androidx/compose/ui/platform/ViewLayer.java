...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView ownerView, DrawChildContainer container, Function1<? super Canvas, Unit> drawBlock, Function0<Unit> invalidateParentLayer) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m2258getCenterSzJe1aQ();
    ...
    public void mo3645updateLayerPropertiesdDxrwY(float scaleX, float scaleY, float alpha, float translationX, float translationY, float shadowElevation, float rotationX, float rotationY, float rotationZ, float cameraDistance, long transformOrigin, Shape shape, boolean clip, RenderEffect renderEffect, long ambientShadowColor, long spotShadowColor, int compositingStrategy, LayoutDirection layoutDirection, Density density) {
        ...
        setPivotX(TransformOrigin.m2253getPivotFractionXimpl(this.mTransformOrigin) * getWidth());
        setPivotY(TransformOrigin.m2254getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(viewLayer, ColorKt.m1931toArgb8_81llA(ambientShadowColor));
            ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(viewLayer, ColorKt.m1931toArgb8_81llA(spotShadowColor));
        }
        if (Build.VERSION.SDK_INT >= 31) {
            ViewLayerVerificationHelper31.INSTANCE.setRenderEffect(this, renderEffect);
        }
        if (CompositingStrategy.m1957equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m1963getOffscreenNrFUSI())) {
            setLayerType(2, null);
        } else if (CompositingStrategy.m1957equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m1962getModulateAlphaNrFUSI())) {
            setLayerType(0, null);
            z = false;
        } else {
            setLayerType(0, null);
    ...
    public boolean mo3640isInLayerk4lQ0M(long position) {
        float fM1639getXimpl = Offset.m1639getXimpl(position);
        float fM1640getYimpl = Offset.m1640getYimpl(position);
        if (this.clipToBounds) {
            return 0.0f <= fM1639getXimpl && fM1639getXimpl < ((float) getWidth()) && 0.0f <= fM1640getYimpl && fM1640getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
            return this.outlineResolver.m3726isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mo3643resizeozmzZPI(long size) {
        int iM4550getWidthimpl = IntSize.m4550getWidthimpl(size);
        int iM4549getHeightimpl = IntSize.m4549getHeightimpl(size);
        if (iM4550getWidthimpl == getWidth() && iM4549getHeightimpl == getHeight()) {
            return;
        }
        float f = iM4550getWidthimpl;
        setPivotX(TransformOrigin.m2253getPivotFractionXimpl(this.mTransformOrigin) * f);
        float f2 = iM4549getHeightimpl;
        setPivotY(TransformOrigin.m2254getPivotFractionYimpl(this.mTransformOrigin) * f2);
        this.outlineResolver.m3727updateuvyYCjk(SizeKt.Size(f, f2));
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM4550getWidthimpl, getTop() + iM4549getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo3642movegyyYBs(long position) {
        int iM4508getXimpl = IntOffset.m4508getXimpl(position);
        if (iM4508getXimpl != getLeft()) {
            offsetLeftAndRight(iM4508getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM4509getYimpl = IntOffset.m4509getYimpl(position);
        if (iM4509getYimpl != getTop()) {
            offsetTopAndBottom(iM4509getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo3641mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM3703calculateInverseMatrixbWbORWo = this.matrixCache.m3703calculateInverseMatrixbWbORWo(this);
            return fArrM3703calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m2107mapMKHz9U(fArrM3703calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m1653getInfiniteF1C5BW0();
        }
        return androidx.compose.ui.graphics.Matrix.m2107mapMKHz9U(this.matrixCache.m3704calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        ...
        if (inverse) {
            float[] fArrM3703calculateInverseMatrixbWbORWo = this.matrixCache.m3703calculateInverseMatrixbWbORWo(this);
            if (fArrM3703calculateInverseMatrixbWbORWo != null) {
                androidx.compose.ui.graphics.Matrix.m2109mapimpl(fArrM3703calculateInverseMatrixbWbORWo, rect);
                return;
            } else {
        ...
        androidx.compose.ui.graphics.Matrix.m2109mapimpl(this.matrixCache.m3704calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo3644transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        androidx.compose.ui.graphics.Matrix.m2118timesAssign58bKbWc(matrix, this.matrixCache.m3704calculateMatrixGrdbGEg(this));
    }

    @Override // androidx.compose.ui.node.OwnedLayer
    /* renamed from: inverseTransform-58bKbWc */
    public void mo3639inverseTransform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        float[] fArrM3703calculateInverseMatrixbWbORWo = this.matrixCache.m3703calculateInverseMatrixbWbORWo(this);
        if (fArrM3703calculateInverseMatrixbWbORWo != null) {
            androidx.compose.ui.graphics.Matrix.m2118timesAssign58bKbWc(matrix, fArrM3703calculateInverseMatrixbWbORWo);
        }
    }
