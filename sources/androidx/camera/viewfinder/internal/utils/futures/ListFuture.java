...
class ListFuture<V> implements ListenableFuture<List<V>> {
    ...
    private final ListenableFuture<List<V>> mResult = CallbackToFutureAdapter.getFuture(new CallbackToFutureAdapter.Resolver<List<V>>() { // from class: androidx.camera.viewfinder.internal.utils.futures.ListFuture.1
    ...
    private void init(Executor executor) {
        addListener(new Runnable() { // from class: androidx.camera.viewfinder.internal.utils.futures.ListFuture.2
        ...
        for (final int i2 = 0; i2 < list.size(); i2++) {
            ...
            listenableFuture.addListener(new Runnable() { // from class: androidx.camera.viewfinder.internal.utils.futures.ListFuture.3
                @Override // java.lang.Runnable
                public void run() {
