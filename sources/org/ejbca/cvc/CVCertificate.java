...
public class CVCertificate extends AbstractSequence implements Signable {
    ...
    public void verify(PublicKey publicKey, String str) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException {
        OIDField objectIdentifier;
        try {
            if (publicKey instanceof CVCPublicKey) {
                objectIdentifier = ((CVCPublicKey) publicKey).getObjectIdentifier();
            } else {
                objectIdentifier = getCertificateBody().getPublicKey().getObjectIdentifier();
            }
            String algorithmName = AlgorithmUtil.getAlgorithmName(objectIdentifier);
