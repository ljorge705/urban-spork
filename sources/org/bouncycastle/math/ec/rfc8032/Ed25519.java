...
public abstract class Ed25519 {
    ...
    private static final byte[] DOM2_PREFIX = {83, 105, 103, 69, ISOFileInfo.FMD_BYTE, ISO7816.INS_INCREASE, 53, 53, 49, 57, 32, 110, ISOFileInfo.FCI_BYTE, 32, 69, ISOFileInfo.FMD_BYTE, ISO7816.INS_INCREASE, 53, 53, 49, 57, 32, 99, ISOFileInfo.FCI_BYTE, 108, 108, 105, 115, 105, ISOFileInfo.FCI_BYTE, 110, 115};
    ...
    private static int checkPoint(PointAccum pointAccum) {
        ...
        F.sqr(pointAccum.x, iArrCreate2);
        F.sqr(pointAccum.y, iArrCreate3);
        F.sqr(pointAccum.z, iArrCreate4);
    ...
    private static boolean checkPointFullVar(byte[] bArr) {
        ...
        if (i == 0 && Integer.MIN_VALUE + iDecode323 >= P[0] - C.RATE_UNSET_INT) {
            return false;
        }
    ...
    private static void invertDoubleZs(PointExtended[] pointExtendedArr) {
        ...
        int[] iArrCreate = F.create();
        F.copy(pointExtendedArr[0].z, 0, iArrCreate, 0);
        ...
        while (true) {
            ...
            F.mul(iArrCreate, pointExtendedArr[i2].z, iArrCreate);
            F.copy(iArrCreate, 0, iArrCreateTable, i2 * 10);
            i = i2;
        }
        ...
        while (i > 0) {
            int i3 = i - 1;
            F.copy(iArrCreateTable, i3 * 10, iArrCreate2, 0);
            F.mul(iArrCreate2, iArrCreate, iArrCreate2);
            F.mul(iArrCreate, pointExtendedArr[i].z, iArrCreate);
            F.copy(iArrCreate2, 0, pointExtendedArr[i].z, 0);
            i = i3;
        }
        F.copy(iArrCreate, 0, pointExtendedArr[0].z, 0);
    }
    ...
    private static void pointLookup(int i, int i2, PointPrecomp pointPrecomp) {
        int i3 = i * FaceDetectorAvcMLKit.FACE_DETECTION_FRAME_WIDTH;
    ...
    private static int[] pointPrecomputeZ(PointAffine pointAffine, int i, PointTemp pointTemp) {
        ...
        while (true) {
            pointCopy(pointExtended, pointPrecompZ);
            F.copy(pointPrecompZ.ymx_h, 0, iArrCreateTable, i2);
            F.copy(pointPrecompZ.ypx_h, 0, iArrCreateTable, i2 + 10);
            F.copy(pointPrecompZ.xyd, 0, iArrCreateTable, i2 + 20);
            F.copy(pointPrecompZ.z, 0, iArrCreateTable, i2 + 30);
            i2 += 40;
            i3++;
            if (i3 == i) {
    ...
    private static void scalarMultStraus128Var(int[] iArr, int[] iArr2, PointAffine pointAffine, int[] iArr3, PointAffine pointAffine2, PointAccum pointAccum) {
        ...
        while (i >= 0) {
            ...
            byte b4 = bArr3[i];
            if (b4 != 0) {
                pointAddVar(b4 < 0, pointPrecompZArr2[(b4 >> 1) ^ (b4 >> Ascii.US)], pointAccum, pointTemp);
            }
            pointDouble(pointAccum);
            i--;
        }
